...
def preprocess_user_input(user_input):
    ...
    user_input = PorterStemmer()
    stemmed_tokenizer = [user_input.stem(word) for word in tokens]
    print(stemmed_tokenizer)
    # Remove duplicacy and sort the user_input
    user_input = list(set(tokens))
    sorted_tokens = sorted(user_input)
    print(sorted_tokens)
   ...
   for intent in intents['intents']:
    if intent['tag']==predicted_class:
       ...
       # choose a random bot response
        bot_response = "Hello. How may I help you"
